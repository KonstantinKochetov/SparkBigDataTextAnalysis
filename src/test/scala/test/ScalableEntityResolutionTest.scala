...
class ScalableEntityResolutionTest extends FunSuite with BeforeAndAfterAll {
  ...
  override protected def beforeAll() {
    ...
    entityResolution.amazonTokens = entityResolution.getTokens(entityResolution.amazonRDD)
    entityResolution.googleTokens = entityResolution.getTokens(entityResolution.googleRDD)
    entityResolutionScalable = new ScalableEntityResolution(sc, dat1, dat2, stopwordsFile, goldStandardFile, entityResolution)
    entityResolutionScalable.amazonTokens = entityResolution.amazonTokens
    entityResolutionScalable.googleTokens = entityResolution.googleTokens
